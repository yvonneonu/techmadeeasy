...
function App() {
  ...
  const getData = useCallback(
    async () => {
      ...
      axios.all([ await axios.get(`https://api.terawork.com/service-categories/sellers-services/computer-software-development`),
           await axios.get(`https://api.terawork.com/resources`)
          ])
     .then(axios.spread((firstResponse, secondResponse, thirdResponse) => {  
      const res = firstResponse.data.data.service_search_results.hits.map((item) => {
        item.isFave = false;
          return item;
        });
        setCardinfo(res);

      const sec secondResponse.data.data.countries
